...
layout = html.Div([
    html.Div([
        ...
        html.H1('Analyse distributionnelle', style={'flex-grow':'2'}),
        html.Div([
...
def update_graph(match, joueur, taille_fenetre, var_mesuree, var_analyse):
    ...
    df_plot['sd'] = df_joueur.loc[df_joueur.date_match != match].groupby('min_fin_fenetre').std(numeric_only = True).reset_index()[var_analyse].to_numpy()
    ...
    df_plot['borne_sup'] = 1.96*df_plot['sd']/np.sqrt(df_plot['nb_points']) + df_plot['moyenne']
    df_plot['borne_inf'] = -1.96*df_plot['sd']/np.sqrt(df_plot['nb_points']) + df_plot['moyenne']
